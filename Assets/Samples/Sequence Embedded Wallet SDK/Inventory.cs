...
namespace Game.Scripts
{
    public class Inventory
    {
        ...
        public void RefreshTokenBalances(TextMeshProUGUI tokenBalanceText)
        {
            GetTokenBalances(null,tokenBalanceText);
        }
        
        private async Task GetTokenBalances(Page page = null, TextMeshProUGUI tokenBalanceText = null)
        {
            if (page == null)
            {
                page = new Page();
            ...
            {
                await GetTokenBalances(balances.page,tokenBalanceText);
            }
            UpdateBalanceUI(tokenBalanceText);
        }
        TextMeshProUGUI tokenBalanceText; // UI Text for displaying balances
        private void UpdateBalanceUI(TextMeshProUGUI tokenBalanceText)
        {
            if (tokenBalanceText == null) return; // Ensure text field is assigned

            if (_tokenBalances.Count == 0)
            {
                tokenBalanceText.text = "No Tokens Found";
                return;
            }

            tokenBalanceText.text = "Your Tokens:\n";
            foreach (var balance in _tokenBalances)
            {
                tokenBalanceText.text += $"Token {balance.Key}: {balance.Value.balance}\n";
            }
        }
